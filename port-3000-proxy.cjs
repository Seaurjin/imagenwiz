...
const TARGET = {
  ...
  port: parseInt(process.env.TARGET_PORT || '5000', 10)
};
...
// Write a startup file to indicate we tried to start
const fs = require('fs');
fs.writeFileSync('proxy-startup.log', `Proxy startup attempt at ${new Date().toISOString()}\n`);

// Start the proxy server
startProxyServer().catch(error => {
  console.error('Failed to start proxy server:', error);
  fs.appendFileSync('proxy-startup.log', `Error: ${error.message}\n${error.stack}\n`);
  process.exit(1);
});